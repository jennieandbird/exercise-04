...
play_wordle <- function(solution, valid_list, num_guesses = 6){
  ...
  letters_remaining <- setdiff(paste(LETTERS), paste(guess1vector))
...
evaluate_guess <- function(guess1, solution){
  ...
  # make a loop that goes letter by letter, assigning +, *, or - to say if the letter is correct AND in the right place, correct but not in the right place, or incorrect. Save this info as a vector, like "*, -, -, -, *"
    # identify the incorrect letters
  incorrectletters <- setdiff(paste(guess1vector), paste(solutionvector))
    # identify the letters that exist in the word (in any order)
  lettersinword <- intersect(guess1vector, solutionvector)
    # identify the spots that are correct letter and correct order
  perfectletters <- (guess1vector == solutionvector)
}
